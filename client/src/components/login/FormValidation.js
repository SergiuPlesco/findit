...
const isRegistrationValid = (user) => {
  try {
    const verifiedUser = registerForm.validate(
      user,
...
const isLoggingValid = (user) => {
  try {
    const verifiedUser = loginForm.validate(
      user,
      customValidationErrorMessages
    );
    return verifiedUser;
...
const isEmailValid = (userEmail) => {
  try {
    const verifiedEmail = forgotPasswordForm.validate(
      userEmail,
      customValidationErrorMessages
    );
...
const isNewPasswordValid = (newPassword) => {
  try {
    const verifiedPassword = resetPasswordForm.validate(
      newPassword,
      customValidationErrorMessages
    );
