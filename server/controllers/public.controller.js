...
const getCompaniesByCategory = async (req, res) => {
	...
	try {
		...
		return res
			.status(500)
			.json({ success: false, error: `Could not retrieve categories. ${error.message}` });
	}
};

export default { getCityBrandsAndCategories, getCompanyDetails, getCompaniesByCategory };
